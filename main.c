...
main()
{
	...
	while (1) {
		...
		for (i = 0; i < 56; i++) {
			reset_pixel(screen, 0, i);
		}
		for (i = 0; i < height; i++) {
			set_pixel(screen, 0, i);
		}
		for (i = height + space; i < 56; i++) {
			set_pixel(screen, 0, i);
		}
...
bool
getPixel(SCREEN* data, unsigned char x, unsigned char y)
{
	return getBit(getSector(data, x / 8, y), 7 - (x % 8));
}

bool
getBit(unsigned char byte, unsigned char number)
{
	return (byte >> number) & 0x01;
}

unsigned char
getSector(SCREEN* data, unsigned char x, unsigned char y)
{
	return data[(y * 12) + x];
}
